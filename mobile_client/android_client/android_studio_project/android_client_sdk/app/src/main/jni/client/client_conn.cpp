...
namespace gim
{
	...
	int CliConn::setPubKey(const std::string& keyfile){
		m_pubkey = keyfile;
		if (m_rsa){
			delete m_rsa;
			m_rsa = NULL;
	...
	void CliConn::setToken(const std::string& token){
		m_token = token;
	}
	...
	{
		if (!m_pubkey.empty() && m_rsa == NULL){
			m_rsa = new ef::RSAEnt();
			int ret =  m_rsa->initPublicKey(m_pubkey);
			if(ret < 0){
				SDK_LOG(LOG_LEVEL_ERROR, "cid=%s, init rsa pubkey %s fail ret=%d", m_cid.c_str(), m_pubkey.c_str(), ret);
				delete m_rsa;
				m_rsa = NULL;
				OnLoginFail(GIM_RSA_INIT_ERROR);
				return -1;
			}
		}
	...
	int32 CliConn::login()
	{
		...
		std::string encbody;
		if (m_rsa){
			int ret = m_rsa->encryptByPublicKey(body, encbody);
			if (ret < 0){
				SDK_LOG(LOG_LEVEL_ERROR, "cid=%s, CliConn::login encrypt fail ret=%d", m_cid.c_str(), ret);
				setStatus(-1, GIM_RSA_INIT_ERROR, true);
				return -1;
			}
		}
		else{
		...
		ret = sendPacket(LOGIN_REQ, encbody);
		return ret >= 0 ? 0 : -1;
	}
	...
	{
		SDK_LOG(LOG_LEVEL_ERROR, "cid=%s, handleLoginResponse", m_cid.c_str());
		SmartOp op(NULL);
		...
		if (status >= 0)
		{
			std::string sessid = lgresp.sessid();
			if (m_rsa){
				sessid = ef::base64Decode(sessid);
				int ret = m_rsa->decryptByPublicKey(sessid, m_sessid);
				if (ret < 0){
					SDK_LOG(LOG_LEVEL_ERROR, "cid=%s, handleLoginResponse decrypt error ss=%s, ret=%d", m_cid.c_str(), sessid.c_str(), ret);
					OnLoginFail(-1);
					return 0;
				}
				m_key = m_token + m_sessid;
			}
			else{
				m_sessid = sessid;
			}
	...
	int32 CliConn::handleServiceRequest(const std::string& reqbody)
	{
		std::string decreq;
		if (m_rsa){
			if (ef::aesDecrypt(reqbody, m_key, decreq) < 0){
				SDK_LOG(LOG_LEVEL_ERROR, "cid=%s, handleServiceRequest decrypt fail!", m_cid.c_str());
				return GIM_DECRYPT_ERROR;
			}
		}
		else{
			decreq = reqbody;
		...
		g_req_count++;
		PushRequest svreq;
		if (!svreq.ParseFromArray(decreq.data(), decreq.size()))
		{
		...
		std::string enresp;
		if (m_rsa){
			if (ef::aesEncrypt(respbody, m_key, enresp) < 0){
				return GIM_DECRYPT_ERROR;
			}
		}
		else{
