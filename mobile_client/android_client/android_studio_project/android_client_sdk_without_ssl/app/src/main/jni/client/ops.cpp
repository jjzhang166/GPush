...
{
	// class LoginOp
	int32 LoginOp::init(const std::string& srvip, int32 srvport, const std::string& cliver, int32 keepalive_timeout, const std::string& pubkey , const std::string& token)
	{
		m_srvip = srvip;
		m_srvport = srvport;
		m_cliver = cliver;
	...
	int32 LoginOp::process(EventLoop* el)
	{
		...
		if (!conn)
		{
			...
			conn->setToken(m_token);
			//conn->setPubKey(m_pubkey);
		}
		else
		{
